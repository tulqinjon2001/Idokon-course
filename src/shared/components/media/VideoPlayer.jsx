...
/**
 * YouTube video komponenti — oddiy va kengaytirilgan rejimlarni qo'llab-quvvatlaydi.
 * `containerClassName` berilsa oddiy (VideoSection) ko'rinish ishlatiladi.
 */
export default function VideoPlayer({
  ...
  const isVertical =
    vertical !== null ? vertical : isShort || isVerticalUrl(videoUrl);
  ...
  const useSimple = containerClassName !== undefined;
      ...
      <div className={containerClassName}>
        <div
          className={`w-full overflow-hidden rounded-lg ${
            isVertical ? "mx-auto max-w-xs sm:max-w-sm" : ""
          }`}
        >
          <iframe
            title={title}
            ...
            allowFullScreen
            className={`w-full rounded-lg ${isVertical ? "h-[560px]" : heightClass}`}
            loading="lazy"
        ...
        <div className="mt-3 sm:mt-4 flex flex-wrap gap-3">
          {videoUrl && (
            <a
              ...
              className="px-5 py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition"
